...
(function(Ea, P, k) {
	var O = function(h) {
		...
		function ob(a) {
			var b = a.oClasses,
				...
				b = h("<div/>", {
					...
					"class": b.sFilter
				}).append(h("<label class='input_btn_seach'>").append(j)),
				f = function() {
		...
		h.extend(o.ext.classes, {
			...
			sWrapper: "dataTables_wrapper table-responsive",
...
jQuery.extend(jQuery.fn.dataTableExt.oSort, {
	"num-format-pre": function(a) {
		var x = String(a).replace(/<[^>]+>/g, ""); //去除html标记
		var x = x.replace(/(^\s*)|(\s*$)/g, ""); //去除空格
		x = x.replace(/￥/, ""); //去除￥符号
		x = x.replace(/%/, ""); //去除%符号
		return parseFloat(x);
	},

	"num-format-asc": function(a, b) { //正序排序引用方法
		return((a < b) ? -1 : ((a > b) ? 1 : 0));
	},

	"num-format-desc": function(a, b) { //倒序排序引用方法
		return((a < b) ? 1 : ((a > b) ? -1 : 0));
	}
});

jQuery.extend(jQuery.fn.dataTableExt.oSort, {
	"level-pre": function(a) {
		var arr = {
			'S': 10,
			'A': 9,
			'B': 8,
...
Array.prototype.toMapset = function() {
	var obj = {};
...
jQuery.extend(jQuery.fn, {
	dataTableAjax: function(url, opt) {
		var option = {
			sAjaxSource: url,
			serverSide: true,
			fnServerData: function(sSource, aoData, fnCallback, oSettings) {
				...
				postData.pagenum = (aoData.iDisplayStart / aoData.iDisplayLength) + 1;
				postData.sortby = eval('(aoData.mDataProp_' + aoData.iSortCol_0 + ')');
				postData.sorttype = aoData.sSortDir_0;
				postData.search = aoData.sSearch;
				$.get(sSource, postData, function(json) {
					var result = json.result;
					if (result == 2001){
						window.location.href = '/sign-in-m.html';
						return;
					}
					var data = {
						draw: aoData.sEcho,

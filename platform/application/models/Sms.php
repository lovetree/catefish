...
class SmsModel extends BaseModel {
    ...
    const SMS_TYPE_ADMINLOGIN    = 4;          //后台登录
...
//                $code = $this->getCode($codeNumber, $rule);
                ...
                $data['c_time'] = time();
                $data['code'] = 0;
//                !$db->insert('ms_sms', array_fetch($data, array('phone', 'type', 'content', 'code', 'expire', 'c_time')))
                if (!$status['status']){
                    logMessage('sms 发送短信失败;' . $status['msg']);
                    //更新数据错误信息
                    try{
                        $data['errMsg'] = $status['msg'];

                        $sql = "insert into ms_sms (phone, type, content, expire, c_time, code,errMsg) VALUES (".implode(',',$data).")";
                        if (!$db->exec($sql)) {
                    ...
                    $data['code']=$status['obj'];
                    $data['status'] = 1;
                    $sql = "insert into ms_sms (phone, type, content, expire, c_time, code,status) VALUES (".implode(',',$data).")";
                    if (!$db->exec($sql)) {

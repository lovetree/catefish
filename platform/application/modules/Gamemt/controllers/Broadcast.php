...
class BroadcastController extends BaseController {
    ...
    public function init() {
        if (!$this->model){
            $this->model = new \Gamemt\BroadcastModel();
        }
    }
    ...
    public function listAction() {
        ...
        $pageInfo['pageUrl'] = '/gamemt/broadcast/list';
    ...
    public function saveAction() {
        ...
        $request['start_time'] = strtotime($time[0]);
        $request['end_time'] = strtotime($time[1]);
        
        if ($request['end_time'] <= $request['start_time']){
            return $this->failed("日期不合法");
        }

...
class EmailController extends BaseController {
    ...
    public function init() {
        if (!$this->model){
            $this->model = new \Gamemt\EmailModel();
        }
    }
    ...
    public function listAction() {
        ...
        //搜索条件
        $query = $request['search'] ?? false;
        $query_type = $request['query_type'] ?? false;

        if (!empty($query) && !empty($query_type)) {
            $this->getView()->assign('type', $query_type);
            $this->getView()->assign('search', $query);
            switch ($query_type) {
                case 'id':
                    $args['filters']['to_id'] = $query;
                    break;
                case 'type':
                    $args['filters']['type'] = $query;
                    break;
                default:
                    $args['filters']['to_id'] = $query;
                    break;
        ...
        $pageInfo['pageUrl'] = '/gamemt/email/list';

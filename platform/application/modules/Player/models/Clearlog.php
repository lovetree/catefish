<?php
namespace Player;
class ClearlogModel extends \BaseModel {
/**
     * 获取列表
     * @param int $page
     * @param int $pagesize
...
public function lists(int $page = 1, int $pagesize = 15, array $args = []): array {
        $db = $this->DB();
        $select = $db->newSelect('ms_clear_log')
	->select('id')
	->select('address')
	->select('game_id')
	->select('escape_num')
...
/**
    ...
    public function create(array $params = []) {
        $params = array_fetch($params, 'id','address','game_id','escape_num','reason','operator','operate_at');
        $model = $this->DB()->getTable('ms_clear_log');

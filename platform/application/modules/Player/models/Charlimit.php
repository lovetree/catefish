<?php
namespace Player;
class CharlimitModel extends \BaseModel {
/**
     * 获取列表
     * @param int $page
     * @param int $pagesize
...
public function lists(int $page = 1, int $pagesize = 15, array $args = []): array {
        $db = $this->DB();
        $select = $db->newSelect('ms_char_limit')
	->select('id')
	->select('char_limit')
	->select('expired_at')
	->select('created_at')
...
/**
    ...
    public function create(array $params = []) {
        $params = array_fetch($params, 'id','char_limit','expired_at','created_at','status','updated_at');
        $model = $this->DB()->getTable('ms_char_limit');

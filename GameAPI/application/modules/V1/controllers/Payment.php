...
class PaymentController extends \BaseController {
    ...
    public function khbAction()
    {
        ...
        $userData = new Data\UserModel($input["Remark"]);
        $userData->addGold($input['Money'] * 100);
        //推送数据
        $pushData = [
            'cmd' => 'gold',
            'data' => [
                'change' => $input['Money'] * 100,
                'total' => $userData->get('gold'),
                'o_time' => time()
            ]
        ];
        //向客户端推送金币变更消息
        curl_get(config_item('push.ip').'/phppushtouser?userid=' . $input["Remark"] . '&phpdata=' . json_encode($pushData));
    ...
    public function cashLinkAction(){
        $this->succ(['link'=>'http://'.$_SERVER['HTTP_HOST'] . '/xiafen.html', 'expired_at'=>1495111921]);
    }
}

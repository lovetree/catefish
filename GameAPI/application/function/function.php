...
{
    if (preg_match('/^((?:1[0-9]{2})+\d{8})$/', $phone)) {
        return true;
    }
    return false;
}
...
/*    $ch = curl_init();
    ...
    if ($error) {
        return ['code' => 1, 'msg' => $error];
            ...
            return ['code' => 0, 'data' => json_decode($output,true)];
    }
}

function postData($service,$terminal, $params)
{
    $postData['service'] = $service;
    $postData['sign_type'] = 'MD5';
    $postData['timestamp'] = time().'';
    $postData['system_id'] = '12';
    ...
    $sign.=config_item('aPlatform.key');
    $sign = md5($sign);
    $postData['sign'] = $sign;
   /* echo (json_encode($postData));
    echo ('</br>');*/
    $result=curl_post(config_item('aPlatform.apiurl'),json_encode($postData));
    if($result['code']==0 && $result['data']['result_code']!=0)
    {
        $result['code']=1;
        $result['msg']=$result['data']['result_desc'];
    }
